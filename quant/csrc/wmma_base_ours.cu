...
size_t initwmmaBaseOurs() {
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(wmmaBaseOursKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size));

    return smem_max_size;
}

void wmmaBaseOurs(half *A, half *B, half *C, size_t M, size_t N, size_t K) {
