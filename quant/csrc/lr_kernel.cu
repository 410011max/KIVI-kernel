...
__global__ void lrKernel(const half *__restrict__ A, const half *__restrict__ B, half *__restrict__ C, 
...
size_t initLr() {
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(lrKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size));
    ...
    torch::Tensor _C)
{
    ...
    at::Tensor _D = torch::empty({M, N}, options);
    auto D = reinterpret_cast<half *>(_D.data_ptr<at::Half>());

    // Kernel excution
    static size_t smem_max_size = initLr();
    dim3 block(THREADS_PER_BLOCK);
    dim3 grid(BLOCK_STRIDE, div_ceil(M, BLOCK_ROWS),
                div_ceil(N, BLOCK_COLS * BLOCK_STRIDE));

    lrKernel<<<grid, block, smem_max_size>>>(A, B, D, M, N, K);
